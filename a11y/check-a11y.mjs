// Crawls the rendered Quarto site with a headless browser and collects the
...
// Directories under _site that are Quarto/site infrastructure, not content
...
async function main() {
  ...
  if (!skipRender) {
    console.log("Rendering site with Quarto (debug profile, for axe-core output)...");
